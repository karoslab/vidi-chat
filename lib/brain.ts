...
/**
 * Vidi's brain (gbrain over MyWiki) — read + note-taking helpers.
 *
 ...
 *
 * rememberNote: "vidi, remember this: …" writes a note file into MyWiki;
 * the brain-ingest ops job commits and syncs it into the brain on its next
...
export function rememberNote(note: string): string {
  mkdirSync(KARWIKI_NOTES_DIR, { recursive: true });
  const now = new Date();
  const stamp = now
    .toISOString()
    .replace(/[:T]/g, "-")
    .slice(0, 19);
  const filePath = path.join(KARWIKI_NOTES_DIR, `${stamp}.md`);
  ...
  // Tier-2 (S-redact): this note is written into MyWiki and gbrain-synced, so
...
/**
 * Fire-and-forget, debounced run of the brain-ingest ops job (idempotent,
